...
import { formatTokenMillions, exactNumber, formatPercent } from '../lib';
  ...
  return (
    <section className="attribution-coverage panel" aria-labelledby="attribution-coverage-title">
      ...
      <div className="attribution-coverage-summary">
        <article><span>{t('components.attribution-coverage-panel.valid_local_attribution')}</span><strong>{formatTokenMillions(coverage.localAttributedTokens)}</strong><small>{t('components.attribution-coverage-panel.shared_denominator_for_projects_standalone_chats_and')}</small></article>
        <article><span>{t('components.attribution-coverage-panel.attributed_to_projects')}</span><strong>{formatTokenMillions(coverage.namedProjectTokens)}</strong><small>{exactNumber(coverage.namedProjectTokens)} tokens</small></article>
        <article><span>{t('app.standalone_chats')}</span><strong>{formatTokenMillions(coverage.standaloneConversationTokens)}</strong><small>{t('components.attribution-coverage-panel.remaining_after_project_resolution')} · {coverage.standaloneConversations.withLocalEvidence}/{coverage.standaloneConversations.indexed} {t('components.attribution-coverage-panel.with_local_evidence')}</small></article>
        <article><span>{t('app.local_unmatched')}</span><strong>{formatTokenMillions(coverage.unassignedTokens)}</strong><small>{t('components.attribution-coverage-panel.token_facts_outside_all_indexed_chats_and')}</small></article>
        <article className="is-gap"><span>{t('components.attribution-coverage-panel.no_classification_evidence')}</span><strong>{formatTokenMillions(coverage.unattributedTokens)}</strong><small>{t('components.attribution-coverage-panel.never_allocated_proportionally')}</small></article>
      </div>
      ...
      <div className="attribution-equation" aria-label={t('components.attribution-coverage-panel.valid_local_attribution_identity')}>
        <span>{formatTokenMillions(coverage.namedProjectTokens)} {t('components.attribution-coverage-panel.projects_7fd0e9')}</span>
        <b>+</b>
        <span>{formatTokenMillions(coverage.standaloneConversationTokens)} {t('components.attribution-coverage-panel.standalone_dcd45f')}</span>
        <b>+</b>
        <span>{formatTokenMillions(coverage.unassignedTokens)} {t('components.attribution-coverage-panel.local_unmatched')}</span>
        <b>+</b>
        <span>{formatTokenMillions(coverage.unattributedTokens)} {t('components.attribution-coverage-panel.unclassified')}</span>
        <b>=</b>
        <strong>{formatTokenMillions(coverage.localAttributedTokens)} {t('components.attribution-coverage-panel.valid_local_attribution_a4b512')}</strong>
      </div>
      ...
      <div className="attribution-gap-buckets">
        {coverage.gapBuckets.map((bucket) => (
          <article key={bucket.id}>
            ...
            <b>{bucket.tokens < 0 ? '−' : ''}{formatTokenMillions(Math.abs(bucket.tokens))}</b>
      ...
      <footer>
        <span>{t('components.attribution-coverage-panel.account_total')} {coverage.accountTotalTokens === null ? t('components.attribution-coverage-panel.no_official_data') : formatTokenMillions(coverage.accountTotalTokens)} · {t('components.attribution-coverage-panel.compared_independently_with_local_attribution')}</span>

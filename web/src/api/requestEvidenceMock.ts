import { normalizedEvidenceSelection, type RequestEvidenceQuery } from './requestEvidence';
...
export function mockRequestEvidence(query: RequestEvidenceQuery): RequestEvidenceResponse {
  const account = normalizedEvidenceSelection(query.account), model = normalizedEvidenceSelection(query.model);

...
it('preserves unknown-only turn usage and rejects invalid confirmed counts', async () => {
  ...
  const result = { ...query, scope: 'thread_own_retained_turns', selectedAccount: null,
    selectedModel: null, historyComplete: false, nextOffset: null as number | null,
  ...
  result.rows[0].confirmedRequestCount = 0;
  for (const cursor of [0, -1, 0.5, Number.MAX_SAFE_INTEGER + 1]) {
    result.nextOffset = cursor;
    await expect(fetchTurnEvidence(query)).rejects.toThrow('invalid pagination');
  }
  result.nextOffset = null;
  result.rows.push({ ...result.rows[0] });
  await expect(fetchTurnEvidence(query)).rejects.toThrow('duplicate or missing groups');
});

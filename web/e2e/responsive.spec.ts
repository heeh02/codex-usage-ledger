...
test('request evidence traverses three pages and returns to the exact first row', async ({ page }) => {
  ...
  const first = await rows.first().locator('td').allTextContents();
  const firstExact = await rows.first().locator('td[title]').evaluateAll(cells => cells.map(cell => cell.getAttribute('title')));
  ...
  await expect(rows.first().locator('td')).not.toHaveText(first);
  await expect(rows).toHaveCount(100);
  const second = await rows.first().locator('td').allTextContents();
  await panel.getByRole('button', { name: '下一页', exact: true }).click();
  ...
  await expect(rows).toHaveCount(100);
  await expect(rows.first().locator('td')).toHaveText(second);
  await panel.getByRole('button', { name: '首页', exact: true }).click();
  await expect(rows.first().locator('td')).toHaveText(first);
  expect(await rows.first().locator('td[title]').evaluateAll(cells => cells.map(cell => cell.getAttribute('title')))).toEqual(firstExact);
  await expect(panel.getByRole('button', { name: '上一页', exact: true })).toBeDisabled();
  await page.setViewportSize({ width: 560, height: 820 });
